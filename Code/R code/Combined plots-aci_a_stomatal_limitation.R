...
photo = subset(data, comp == 'A')
limitation = subset(data, comp == 'Ls')

# plot boxplots for photosynthesis
p1 = ggplot(photo, aes(Nutrients,response)) + geom_bar(stat = 'identity')
#p1 = p1+geom_boxplot(aes(group=Nutrients),fill="transparent")
p1 = p1+labs(x=expression(Nutrient~concentration~~('%')),y=expression(A~~(mu~mol~m^-2~s^-1)))
...
# plot boxplots for stomatal limitation
p2 = ggplot(limitation, aes(Nutrients,response)) + geom_bar(stat = 'identity')
#p2 = p2+geom_boxplot(aes(group=Nutrients),fill="transparent")
p2 = p2+labs(x=expression(Nutrient~concentration~~('%')),y=expression(Stomatal~limitation~~('%')))
...
# plot p used here is an A-ci curve, the code is in the file 'Aci curve plots'
grid.arrange(p, arrangeGrob(p1, p2, nrow=2), nrow = 1)
