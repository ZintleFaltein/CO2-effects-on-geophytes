...
mod2<-lm(Count~Temp*Cat,data=data)
anova (mod2, test="F")

###########Conduct post-hoc tests
leastsquare = lsmeans(mod2, pairwise~Temp*Cat, adjust="tukey")
library(lsmeans)
library(multcompView)
summary (leastsquare)
...
############Plot boxplot
p<-ggplot(data, aes(x=CO2, y=tuber))
p<-p+ geom_boxplot(outlier.size=0,notch=TRUE)
p
############Axes titles
p<- p + labs(x=expression(Temp~catagories[]),y=expression(Count[])) 
